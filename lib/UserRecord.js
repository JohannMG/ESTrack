...
function updateRecord(_location, _room, _esid) {
	...
	var userExists = (typeof ESID_CACHE[_esid] !== 'undefined'); //is user in cache?
	
	//if (userExists) { console.log('user in cache'); }
	
	//if user does not exsits in cache, check database
	if (!userExists && validESID) {
		...
		pg.connect(dbURL, function (err, client, done) {
			if (err) { console.log(err); }
				...
				//check if user in DB
				checkUserInTable(_esid, _location, _room, client, function (found) {
					userExists = found;
					if (!found) {
						insertUserToDB(_esid, _location, _room, client, function (successful) {
							userExists = successful;
						});
					}
				});
			}
...
function checkUserInTable(_esid, _location, _room, client, _callback) {

	var userFound = false;
	var selectstring = "SELECT (esid) FROM " + dbTable + " WHERE esid=$1";
...
function insertUserToDB(_esid, _location, _room, client, _callback) {
...
function cleanESID_CACHE() {
	...
	console.log("Cache objects cleaned:  " + cleaned + " items in cache: " + inCache);

} //cleanESID_CACHE()

function getCacheCount(_callback) {
	var esidCount = 0;
	for (var key in ESID_CACHE) {
		if (!ESID_CACHE.hasOwnProperty(key)) { continue; }
		esidCount++;
...
exports.setUp = setUp;
exports.clearCache = cleanESID_CACHE;
